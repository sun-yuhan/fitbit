...
var heartRate = document.getElementById(`heartbeat`);

title.text = "Prevent Sleep Paralysis";

if (HeartRateSensor) {
   console.log("This device has a HeartRateSensor! :D");
   ...
   console.log("This device does NOT have a HeartRateSensor! D:");
}
...
function updateFinishView() {
  ...
  document.getElementById("btn-finish").addEventListener("click", () => {
    ...
    // are unloaded.
    document.location.replace("index.view").then(updateSessionView);
  });
  ...
  sessionToggle.addEventListener("click", () => {
    setTimeout(() => {
      ...
      document.onbeforeunload = (evt) => {
        ...
        setTimeout(() => {
          document.location.assign('confirm.view').then(updateFinishView);
        }, VIEW_RESET_DELAY_MS);
      }
...
function sessionViewStart() {
  ...
  document.location.assign('index.view').then(updateSessionView);
}
